      ...
      <div class="row">
        <div class="col-md-12">
          <div class="copyright-section">
            <p>© <a href="#">2017 Transparency IT Consulting</a> (ABN 44 131 923 469)<br/>All rights Reserved</p>
          </div>
        </div>  
      </div>
    </div>
...
var base_url = "<?= $base_url; ?>";
  $(function () {    
    ...
    $("#option_service_provider_yes").click(function(){      
      if( $(this).is(':checked') ) {
        $('#option_service_provider_no').attr('checked', false);
        $('.option-free-text').removeClass('hidden');
        $('#input-service-provider').attr("required", "true");
      }else{
        $('.option-free-text').addClass('hidden');        
      }
    });

    $("#option_service_provider_no").click(function(){
      if( $(this).is(':checked') ) {
        $('#option_service_provider_yes').attr('checked', false);
        $('#input-service-provider').removeAttr('required');
        $('.option-free-text').addClass('hidden');
      }
    });
    ...
    function load_job_role_description( selected_id ){
      $.ajax({
         type: "GET",
         dataType: "json",
         url: base_url + 'pages/ajax_job_roles_description',     
         ...
         success: function(o)
         {            
            $(".job-role-description").html(o.description);       
         }
      });
    }

    $("#register-login-button").click(function(){
      location.href = base_url + 'member_login';
    });

    $("#job-roles-list").change(function(){
      var selected_id = $(this).val();
      load_job_role_description(selected_id);
      
    });

    <?php if( $load_job_role_script ){ ?>
      load_job_role_description(1);
    <?php } ?>
